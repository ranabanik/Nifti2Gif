"""
will save the gif in the same folder of the nii.gz image
"""

import os
import nibabel as nb
import numpy as np
import matplotlib.pyplot as get_cmap
from imageio import mimwrite
from skimage.transform import resize
import glob

path = r'C:\Data\Folder\Subfolder\5634'
filepath = os.path.join(path)
filepath = glob.glob(os.path.join(filepath,'*.gz')) #todo: make it work with .nii extension
# CT_path = filepath[0]
filename = filepath[0] #todo:
mode = 'normal'
fps = 20
size = 1.0
...
gif_file = write_gif_normal(filename,size,fps)